...
class SimpleMesh {
 public:
      ...
      for (unsigned int u = 0; u < sensor.getDepthImageWidth(); ++u) {
        ...
        if (depth == MINF) {
          ...
          m_vertices[idx].position =
              ...
              Vector4f((u - cX) / fovX * depth, (v - cY) / fovY * depth, depth,
                       1.0f);
  ...
  bool loadMesh(const std::string& filename) {
    ...
    if (std::string(string1).compare("COFF") == 0) {
      ...
      for (unsigned int i = 0; i < numV; i++) {
        ...
        v.color =
            Vector4uc((unsigned char)colorInt.x(), (unsigned char)colorInt.y(),
    ...
    for (unsigned int i = 0; i < m_vertices.size(); i++) {
      ...
      if (vertex.position.allFinite())
        outFile << vertex.position.x() << " " << vertex.position.y() << " "
                << vertex.position.z() << " " << int(vertex.color.x()) << " "
                << int(vertex.color.y()) << " " << int(vertex.color.z()) << " "
   ...
   * transforming the vertex positions of mesh1 with transformation 'pose1to2'.
   */
  static SimpleMesh joinMeshes(const SimpleMesh& mesh1, const SimpleMesh& mesh2,
  ...
  static SimpleMesh cylinder(const Vector3f& p0, const Vector3f& p1,
    ...
    for (unsigned i = 0; i <= stacks; i++)
      for (unsigned i2 = 0; i2 < slices; i2++) {
        ...
        v.position = Vector4f{p0.x() + radius * cosf(theta),
