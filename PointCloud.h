...
class PointCloud {
 public:
  ...
  PointCloud(const SimpleMesh& mesh) {
    ...
    m_rgba.reserve(nVertices);
    for (const auto& vertex : vertices) {
      m_points.push_back(Vector3f{vertex.position.x(), vertex.position.y(),
                                  vertex.position.z()});
      m_rgba.push_back(Vector4uc{vertex.color.x(), vertex.color.y(),
...
#pragma omp parallel for
    ...
    // We need to compute derivatives and then the normalized normal vector (for
    // valid pixels).
...
#pragma omp parallel for
    for (int v = 1; v < height - 1; ++v) {
      for (int u = 1; u < width - 1; ++u) {
        ...
        const float du = depthMap[idx + 1] - depthMap[idx - 1];
        const float dv = depthMap[idx + width] - depthMap[idx - width];
        if (!std::isfinite(du) || !std::isfinite(dv) || abs(du) > maxDistance ||
            abs(dv) > maxDistance) {
          normalsTmp[idx] = Vector3f(MINF, MINF, MINF);
          continue;
        }

        // TODO: Compute the normals using the cross product of approximate
        // tangent vectors.
  ...
  // TODO PROJECT: get RGBA VALUE
  std::vector<Vector4uc>& getRGBA() { return m_rgba; }

  const std::vector<Vector4uc>& getRGBA() const { return m_rgba; }
