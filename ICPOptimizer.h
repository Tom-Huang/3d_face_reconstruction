...
class PointToPointConstraint {
 public:
  ...
  bool operator()(const T* const pose, T* residuals) const {
    ...
    // The resulting 3D residual should be stored in residuals array. To apply
    // the pose increment (pose parameters) to the source point, you can use the
    // PoseIncrement class. Important: Ceres automatically squares the cost
    // function.
    T pose_c[6] = {pose[0], pose[1], pose[2], pose[3], pose[4], pose[5]};
    PoseIncrement<T> pose_incre(pose_c);
    T p_transform[3];
    T t_sourcePoint[3];
    T t_targetPoint[3];
    fillVector(m_sourcePoint, t_sourcePoint);
    fillVector(m_targetPoint, t_targetPoint);
    pose_incre.apply(t_sourcePoint, p_transform);
    residuals[0] =
        T(m_weight) * T(LAMBDA) * (p_transform[0] - t_targetPoint[0]);
    residuals[1] =
        T(m_weight) * T(LAMBDA) * (p_transform[1] - t_targetPoint[1]);
    residuals[2] =
        T(m_weight) * T(LAMBDA) * (p_transform[2] - t_targetPoint[2]);
...
class PointToPlaneConstraint {
 public:
  ...
  bool operator()(const T* const pose, T* residuals) const {
    ...
    // The resulting 1D residual should be stored in residuals array. To apply
    // the pose increment (pose parameters) to the source point, you can use the
    // PoseIncrement class. Important: Ceres automatically squares the cost
    // function.
    T pose_c[6] = {pose[0], pose[1], pose[2], pose[3], pose[4], pose[5]};
    ...
    fillVector(m_sourcePoint, t_sourcePoint);
    fillVector(m_targetPoint, t_targetPoint);
    fillVector(m_targetNormal, t_targetNormal);
    pose_incre.apply(t_sourcePoint, p_transform);

    residuals[0] = ((p_transform[0] - t_targetPoint[0]) * t_targetNormal[0] +
                    (p_transform[1] - t_targetPoint[1]) * t_targetNormal[1] +
                    (p_transform[2] - t_targetPoint[2]) * t_targetNormal[2]) *
                   T(m_weight) * T(LAMBDA);
  ...
  void prepareConstraints(const std::vector<Vector3f>& sourcePoints,
    ...
    for (unsigned i = 0; i < nPoints; ++i) {
      ...
      if (match.idx >= 0) {
        ...
        // TODO: Create a new point-to-point cost function and add it as
        // constraint (i.e. residual block) to the Ceres problem.
        ceres::CostFunction* cost_function_pp =
            PointToPointConstraint::create(sourcePoint, targetPoint, 1);
        // new ceres::AutoDiffCostFunction<PointToPointConstraint, 3, 6>(new
        // PointToPointConstraint(sourcePoint, targetPoint, 1));

        problem.AddResidualBlock(cost_function_pp, NULL,
        ...
        if (m_bUsePointToPlaneConstraints) {
          ...
          // TODO: Create a new point-to-plane cost function and add it as
          // constraint (i.e. residual block) to the Ceres problem.
          ceres::CostFunction* cost_function_ppl =
              PointToPlaneConstraint::create(sourcePoint, targetPoint,
                                             targetNormal, 1);
          // new ceres::AutoDiffCostFunction<PointToPlaneConstraint, 1, 6>(new
          // PointToPlaneConstraint(sourcePoint, targetPoint, targetNormal, 1));
          problem.AddResidualBlock(cost_function_ppl, NULL,
                                   poseIncrement.getData());
        }
      }
    }
